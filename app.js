...
const defaults={vendors:structuredClone(MASTER_VENDORS),vendorCatalogCleared:false,banks:['彰化銀行','板信銀行'],methods:['現金','支票','郵寄支票','轉帳','匯款'],checks:{'彰化銀行':[],'板信銀行':[]},payments:[]};
...
function migrate(input){
  ...
  const vendorMap=new Map(source.vendorCatalogCleared===true?[]:MASTER_VENDORS.map(v=>[String(v.code),{...v}]));
  ...
  n.vendorCatalogCleared=source.vendorCatalogCleared===true;
...
async function clearAllVendorsSafely(){
  const count=db.vendors.length;
  if(!count)return toast('目前沒有廠商資料可以清除');
  if(!confirm(`確定要清除全部 ${count} 家廠商嗎？\n\n只會移除廠商基本資料與新增付款下拉清單。\n歷史付款中的廠商代號、名稱、付款、照片與簽名都會保留。\n系統會先建立完整內部備份。`))return;
  try{if(typeof window.shuangfaInternalBackup==='function')await window.shuangfaInternalBackup('清除全部廠商前完整備份');else throw new Error('完整內部備份功能尚未就緒')}catch(error){console.error('清除廠商前備份失敗',error);return alert('完整內部備份失敗，系統沒有清除任何廠商資料。')}
  const typed=prompt('為避免誤刪，請輸入「清除廠商」四個字：','');
  if(typed!=='清除廠商')return toast('輸入不正確，已取消清除廠商');
  const before=structuredClone(db);
  db.vendors=[];
  db.vendorCatalogCleared=true;
  if(!await persistDatabaseSafely(before,'廠商清除保存失敗，原資料已保留'))return;
  editingVendorCode='';$('#vendorCodeManage').value='';$('#vendorNameManage').value='';$('#saveVendorManage').textContent='新增';
  renderLists();renderVendorManager();
  toast(`已清除 ${count} 家廠商，歷史付款資料已保留`);
  alert(`清除完成。\n\n已清除：廠商基本資料與新增付款下拉清單。\n已保留：歷史付款、照片、簽名與完整內部備份。`);
}
...
$('#clearAllVendorsBtn').onclick=clearAllVendorsSafely;
...
function fullDatabaseEvidenceShape(data){return JSON.stringify({payments:fullBackupEvidenceShape(data),vendors:stableEvidenceValue(data?.vendors||[]),vendorCatalogCleared:Boolean(data?.vendorCatalogCleared),banks:stableEvidenceValue(data?.banks||[]),methods:stableEvidenceValue(data?.methods||[]),checks:stableEvidenceValue(data?.checks||{}),auditLogs:stableEvidenceValue(data?.auditLogs||[]),correctionLogs:stableEvidenceValue(data?.correctionLogs||[])})}
