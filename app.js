...
function applySystemName(){const name=getSystemName();const h=$('#systemNameHeader');if(h)h.textContent=name;document.title=`${name} V8.3 RC Build 0287・簽名照片保存檢修版`;const loginTitle=document.querySelector('#loginSystemName');if(loginTitle)loginTitle.textContent=name;const apple=document.querySelector('meta[name="apple-mobile-web-app-title"]');if(apple)apple.setAttribute('content',name.slice(0,12))}
...
if('serviceWorker' in navigator){
  window.addEventListener('load',async()=>{
    try{
      swRegistration=await navigator.serviceWorker.register('./sw.js?v=83287',{scope:'./',updateViaCache:'none'});
      updateOfflineStatus();
    }catch(err){
...
window.shuangfaInternalBackup=async function(reason='系統自動備份'){
  ...
  const payload={app:getSystemName(),version:'V8.3 RC Build 0287・內部自動備份版',backupAt,reason,data:structuredClone(db),settings:structuredClone(settings)};
