...
function applySystemName(){const name=getSystemName();const h=$('#systemNameHeader');if(h)h.textContent=name;document.title=`${name} V8.3 DEV Build 025.9・簽名照片保存檢修版`;const loginTitle=document.querySelector('#loginSystemName');if(loginTitle)loginTitle.textContent=name;const apple=document.querySelector('meta[name="apple-mobile-web-app-title"]');if(apple)apple.setAttribute('content',name.slice(0,12))}
...
function fileData(f){return new Promise((resolve,reject)=>{if(!f)return reject(new Error('沒有選擇圖片'));if(f.type&&!String(f.type).startsWith('image/'))return reject(new Error('請選擇圖片檔'));if(f.size>15*1024*1024)return reject(new Error('單張照片請小於 15MB'));const reader=new FileReader();reader.onload=()=>{const image=new Image();image.onload=()=>{try{const max=1800,scale=Math.min(1,max/Math.max(image.width,image.height)),canvas=document.createElement('canvas');canvas.width=Math.max(1,Math.round(image.width*scale));canvas.height=Math.max(1,Math.round(image.height*scale));const context=canvas.getContext('2d');if(!context)throw new Error('照片處理元件無法啟動');context.imageSmoothingEnabled=true;context.imageSmoothingQuality='high';context.fillStyle='#fff';context.fillRect(0,0,canvas.width,canvas.height);context.drawImage(image,0,0,canvas.width,canvas.height);let output='';try{output=canvas.toDataURL('image/webp',.90)}catch{}if(!output||output==='data:,')output=canvas.toDataURL('image/jpeg',.92);if(!output||output==='data:,')throw new Error('照片轉檔失敗');resolve(output)}catch(error){reject(error)}};image.onerror=()=>reject(new Error('照片讀取失敗，請重新拍攝'));image.src=String(reader.result||'')};reader.onerror=()=>reject(new Error('照片檔案讀取失敗'));reader.readAsDataURL(f)})}
