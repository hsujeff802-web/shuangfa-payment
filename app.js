...
function updateOfflineStatus(){
  ...
  el.innerHTML=navigator.onLine
    ? '目前：<b>已連線</b>。目前版本 V8.3 Build 0307，可檢查新版；斷網後仍可使用。'
    : '目前：<b>離線使用中</b>。目前版本 V8.3 Build 0307；新增、查詢、簽名、列印與備份仍可操作。';
}
...
if('serviceWorker' in navigator){
  window.addEventListener('load',async()=>{
    try{
      swRegistration=await navigator.serviceWorker.register('./sw.js?v=83404',{scope:'./',updateViaCache:'none'});
...
window.shuangfaInternalBackup=async function(reason='系統自動備份'){
  ...
  const payload={app:getSystemName(),version:'V8.3 Build 0307・內部自動備份版',backupAt,reason,data:structuredClone(db),settings:structuredClone(settings)};
  let saved=false;
  try{
