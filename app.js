...
const IDB_NAME='shuangfa_payment_media_v1_rc',IDB_STORE='database',IDB_DATA_KEY='db',IDB_BACKUP_KEY='latest-full-backup';
...
async function readIndexedDBEntry(key=IDB_DATA_KEY){const idb=await openAppDB();return new Promise((resolve,reject)=>{const tx=idb.transaction(IDB_STORE,'readonly'),req=tx.objectStore(IDB_STORE).get(key);req.onsuccess=()=>{idb.close();resolve(req.result||null)};req.onerror=()=>{idb.close();reject(req.error||new Error('IndexedDB 讀取失敗'))}})}
...
async function writeToIndexedDB(value,key=IDB_DATA_KEY){const idb=await openAppDB();return new Promise((resolve,reject)=>{const tx=idb.transaction(IDB_STORE,'readwrite');tx.objectStore(IDB_STORE).put({schemaVersion:2,savedAt:new Date().toISOString(),data:structuredClone(value)},key);tx.oncomplete=()=>{idb.close();resolve(true)};tx.onerror=()=>{idb.close();reject(tx.error||new Error('IndexedDB 儲存失敗'))}})}
...
function loadSettings(){const base={autoBackup:true,systemName:'雙發付款管理系統',homeLabels:{payment:'新增付款',settlement:'查詢付款資料',reminder:'支票管理',report:'報表中心'}};try{let current=JSON.parse(localStorage.getItem(SETTINGS_KEY)||'null');if(!current||typeof current!=='object')current={};const migrationMark='shuangfa_payment_v52_rc_settings_migrated';if(!localStorage.getItem(migrationMark)){try{const old=JSON.parse(localStorage.getItem('shuangfa_payment_v52_settings')||'null');const carry=['systemName','homeLabels','autoBackup','voiceEnabled','voiceErrors','voiceSuccess','voiceBackup','voiceDue','voiceVolume','voiceRate','loginEnabled','loginMode','loginMusicData','loginMusicName','loginText','logoutEnabled','logoutMode','logoutMusicData','logoutMusicName','logoutText','logoutPlayMode'];if(old&&typeof old==='object')carry.forEach(key=>{if(current[key]===undefined&&old[key]!==undefined)current[key]=old[key]});localStorage.setItem(migrationMark,'1')}catch(error){console.warn('RC 語音設定移轉略過',error)}}return {...base,...current,homeLabels:{...base.homeLabels,...(current.homeLabels||{})}}}catch{return base}}
function save(){
...
function applySystemName(){const name=getSystemName();const h=$('#systemNameHeader');if(h)h.textContent=name;document.title=`${name} V8.3 RC Build 0282・簽名照片保存檢修版`;const loginTitle=document.querySelector('#loginSystemName');if(loginTitle)loginTitle.textContent=name;const apple=document.querySelector('meta[name="apple-mobile-web-app-title"]');if(apple)apple.setAttribute('content',name.slice(0,12))}
...
async function saveAndVerifyDatabase(){await saveWithStorageRecovery();const saved=await readPersistedDatabase();if(saved.payments.length!==db.payments.length)throw new Error('資料保存筆數驗證失敗');return saved}
...
function speakSignature(text){try{if(window.shuangfaSpeak)window.shuangfaSpeak(text,'success');else speak(text)}catch(error){console.warn('簽名語音播放失敗',error)}}
...
$('#photosNext').onclick=()=>{updateSignaturePaymentInfo();signatureConfirmed=false;stopSignatureReminder();show('signature');setTimeout(()=>{if(isSignaturePageActive()&&!hasSignature)speakSignature('請先確認收款金額無誤，再於下方簽名。')},350)};
...
function begin(e){e.preventDefault();drawing=true;hasSignature=false;signatureData='';signatureConfirmed=false;stopSignatureReminder();lastPoint=pos(e);try{c.setPointerCapture(e.pointerId)}catch{}}
...
function end(e){if(!drawing)return;drawing=false;try{c.releasePointerCapture(e.pointerId)}catch{};hasSignature=canvasHasInk();signatureData=hasSignature?exportSignatureImage():'';if(hasSignature)startSignatureReminder(true);else stopSignatureReminder()}
...
function renderSettings(){$('#systemNameInput').value=getSystemName();const hl={payment:'新增付款',settlement:'查詢付款資料',reminder:'支票管理',report:'報表中心',...(settings.homeLabels||{})};$('#homeLabelPayment').value=hl.payment;$('#homeLabelSettlement').value=hl.settlement;$('#homeLabelReminder').value=hl.reminder;$('#homeLabelReport').value=hl.report;$('#autoBackupToggle').checked=settings.autoBackup;renderBackupStatus();renderInternalBackupStatus();renderStorageStatus();$('#bankChips').innerHTML=db.banks.map((v,i)=>`<button class="chip" data-db="${i}">${esc(v)} ×</button>`).join('');$('#methodChips').innerHTML=db.methods.map((v,i)=>`<button class="chip" data-dm="${i}">${esc(v)} ×</button>`).join('');$$('[data-db]').forEach(x=>x.onclick=()=>{const n=db.banks[+x.dataset.db];db.banks.splice(+x.dataset.db,1);delete db.checks[n];save();renderSettings()});$$('[data-dm]').forEach(x=>x.onclick=()=>{const v=db.methods[+x.dataset.dm];if(['現金','支票','郵寄支票','轉帳','匯款'].includes(v))return toast('基本付款方式不可刪除');db.methods.splice(+x.dataset.dm,1);save();renderSettings()})}
...
function createOpeningBackup(){try{if(!settings.autoBackup)return;const day=localDate(),tag='open-'+day,a=JSON.parse(localStorage.getItem(BACKUP_KEY)||'[]');if(a.some(x=>x.tag===tag))return;const slim=structuredClone(db);slim.payments=(slim.payments||[]).map(x=>({...x,invoicePhotos:[],checkPhoto:'',signatureData:''}));a.unshift({at:new Date().toISOString(),tag,reason:'開啟系統自動備份',data:slim});localStorage.setItem(BACKUP_KEY,JSON.stringify(a.slice(0,7)));localStorage.setItem('shuangfa_last_auto_backup',new Date().toISOString())}catch(e){console.error('開啟自動備份失敗',e)}}
...
if('serviceWorker' in navigator){
  window.addEventListener('load',async()=>{
    try{
      swRegistration=await navigator.serviceWorker.register('./sw.js?v=83282',{scope:'./',updateViaCache:'none'});
      updateOfflineStatus();
    }catch(err){
...
async function clearPaymentDataSafely(){
  ...
  try{if(typeof window.shuangfaInternalBackup==='function')await window.shuangfaInternalBackup('清除付款資料前完整備份');else await writeToIndexedDB(db)}catch(error){console.error('清除前備份失敗',error);return alert('完整內部備份失敗，系統沒有清除任何付款資料。')}
...
window.shuangfaInternalBackup=async function(reason='系統自動備份'){
  ...
  const payload={app:getSystemName(),version:'V8.3 RC Build 0282・內部自動備份版',backupAt,reason,data:structuredClone(db),settings:structuredClone(settings)};
  if(typeof writeToIndexedDB==='function'){
    await writeToIndexedDB(payload,IDB_BACKUP_KEY);
    if(typeof readFromIndexedDB==='function'){
      const verified=await readFromIndexedDB(IDB_BACKUP_KEY);
      const sourcePayments=Array.isArray(payload.data?.payments)?payload.data.payments:[];
      const savedPayments=Array.isArray(verified?.data?.payments)?verified.data.payments:[];
      const mediaCount=list=>list.reduce((sum,p)=>sum+(p.invoicePhotos||[]).filter(Boolean).length+(p.checkPhoto?1:0)+(p.signatureData?1:0),0);
      if(!verified?.data||savedPayments.length!==sourcePayments.length||mediaCount(savedPayments)!==mediaCount(sourcePayments))throw new Error('內部備份讀回驗證失敗');
    }
  }else{
    await save();
  }
