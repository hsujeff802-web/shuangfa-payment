...
function applySystemName(){const name=getSystemName();const h=$('#systemNameHeader');if(h)h.textContent=name;document.title=`${name} V8.3 Build 0311・iPad授權保存修正版`;const loginTitle=document.querySelector('#loginSystemName');if(loginTitle)loginTitle.textContent=name;const apple=document.querySelector('meta[name="apple-mobile-web-app-title"]');if(apple)apple.setAttribute('content',name.slice(0,12))}
...
function updateOfflineStatus(){
  ...
  el.innerHTML=navigator.onLine
    ? '目前：<b>已連線</b>。目前版本 V8.3 Build 0311，可檢查新版；斷網後仍可使用。'
    : '目前：<b>離線使用中</b>。目前版本 V8.3 Build 0311；新增、查詢、簽名、列印與備份仍可操作。';
}
...
if('serviceWorker' in navigator){
  window.addEventListener('load',async()=>{
    try{
      swRegistration=await navigator.serviceWorker.register('./sw.js?v=83408',{scope:'./',updateViaCache:'none'});
...
window.shuangfaInternalBackup=async function(reason='系統自動備份'){
  ...
  const payload={app:getSystemName(),version:'V8.3 Build 0311・內部自動備份版',backupAt,reason,data:structuredClone(db),settings:structuredClone(settings)};
  let saved=false;
  try{
