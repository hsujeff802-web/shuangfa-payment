...
function applySystemName(){const name=getSystemName();const h=$('#systemNameHeader');if(h)h.textContent=name;document.title=`${name} V8.3 DEV Build 0260・簽名照片保存檢修版`;const loginTitle=document.querySelector('#loginSystemName');if(loginTitle)loginTitle.textContent=name;const apple=document.querySelector('meta[name="apple-mobile-web-app-title"]');if(apple)apple.setAttribute('content',name.slice(0,12))}
...
async function saveWithStorageRecovery(currentId){try{return save()}catch(err){if(err?.code!=='STORAGE_QUOTA'&&err?.message!=='STORAGE_QUOTA')throw err;toast('手機空間不足，正在縮小照片與簽名後重試…');speak('手機儲存空間不足，系統正在縮小照片與簽名後重新儲存。');const changed=await optimizeStoredPhotos();try{const ok=save();renderStorageStatus();if(changed){toast(`已縮小 ${changed} 份照片或簽名並完成儲存`);window.shuangfaSpeak?window.shuangfaSpeak('影像縮小完成，付款資料已儲存。','success'):speak('影像縮小完成，付款資料已儲存。')}return ok}catch(err2){if(err2?.code!=='STORAGE_QUOTA'&&err2?.message!=='STORAGE_QUOTA')throw err2;alert('手機瀏覽器可用儲存空間仍不足，因此這筆資料尚未存入。\n\n請先回到「系統設定」按「立即完整備份」，再刪除手機不需要的照片或清理瀏覽器暫存後重試。系統不會刪除已保存的付款照片。');speak('儲存空間仍然不足，請先備份並清理手機空間。');throw new Error('手機儲存空間不足；請先備份並清理手機空間。')}}}
